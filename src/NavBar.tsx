...
const getRandWithin = (high: number, low = 0) => low + Math.round(Math.random() * high);
...
function NavBar() {
    ...
    function draw() {
        if (canvas.current) {
            ...
            while (points > 0) {
                canvas.current.lineTo(getRandWithin(innerWidth), getRandWithin(innerHeight));
                points--;
            }
            canvas.current.fillStyle = colors[Math.floor(Math.random() * colors.length)];
